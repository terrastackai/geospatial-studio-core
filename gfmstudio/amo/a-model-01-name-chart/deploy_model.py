...
def deploy_model(
    ...
    if dry_run:
        ...
        print(yaml_content)
        print("=" * 80)
        deployment_type = "KServe InferenceService" if enable_kserve else "Standard Deployment"
...
Examples:
  ...
  python deploy_model.py my-model --namespace geospatial-studio \\
    ...
    parser.add_argument("--enable-kserve", action="store_true", 
                       help="Use KServe InferenceService instead of standard Deployment")
    parser.add_argument("--image-repository", default="us.icr.io/gfmaas/vllm-small",
                       help="Container image repository")
    parser.add_argument("--image-tag", default="v0.0.6",
                       help="Container image tag")
    parser.add_argument("--image-pull-secret", default="my-registry-secret",
                       help="Name of image pull secret")
    parser.add_argument("--service-account", default="default",
                       help="Service account name")
    parser.add_argument("--models-pvc", default="vllm-models-pvc",
                       help="PVC name for models storage")
    parser.add_argument("--inference-shared-pvc", default="inference-shared-pvc",
                       help="PVC name for shared inference data")
    parser.add_argument("--gpu-count", default="1",
                       help="Number of GPUs to request")
    parser.add_argument("--cpu-limit", default="2000m",
                       help="CPU limit")

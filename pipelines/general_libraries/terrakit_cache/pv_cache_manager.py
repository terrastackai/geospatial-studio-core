...
class TerrakitPVCacheManager:
    ...
    def __init__(
        ...
        self.max_cache_size_bytes = max_cache_size_gb * 1024**3 if max_cache_size_gb else None
        ...
        try:
            ...
            if original_path and imputed_path:
                if Path(original_path).exists() and Path(imputed_path).exists():
                    ...
                    logger.warning(f"⚠️ Cache metadata exists but files missing")
        ...
        try:
            ...
            if self.max_cache_size_bytes:
                ...
                if cache_size > self.max_cache_size_bytes:
                    logger.warning(f"⚠️ Cache size limit reached, cleaning old entries...")
                    self._cleanup_old_entries()
            
            # Get PV paths for cached files
            date = metadata.get("date", "unknown")
            original_pv_path, imputed_pv_path = self._get_cache_file_paths(cache_key, date)
            
            # Copy files to PV cache
            logger.info(f"📤 Caching files to PV...")
            ...
            try:
                ...
                logger.info(f"✅ Cached files in PV: {cache_key[:16]}... (TTL: {self.cache_ttl_seconds}s)")
                return True
            except (AttributeError, ConnectionError) as e:

...
```bash
c3_create_operator --repository us.icr.io/gfmaas --dockerfile_template_path Dockerfile.template --log_level DEBUG --version v1.0.2-aug8-test0 --local_mode terratorch_inference.py gfm_logger gfm_data_processing terratorch_inference_utils.py
```

4. Push the image to a container registry from where it can be deployed:
```bash
docker push us.icr.io/gfmaas/template_process:v1.0-test
```
